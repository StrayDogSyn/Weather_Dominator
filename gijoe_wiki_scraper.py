...
class GIJoeWikiScraper:
    ...
    def scrape_vehicle_list_page(self) -> List[Dict[str, Any]]:
        ...
        for table in tables:
            ...
            header_texts = [
                th.get_text().strip().lower() for th in headers.find_all(["th", "td"])
            ]

            # Check if this looks like a vehicle table
            if any(
                keyword in " ".join(header_texts)
                for keyword in ["vehicle", "name", "year", "pilot", "driver"]
            ):
            ...
            if "character" in section_text:
                ...
                while next_element and next_element.name not in ["h1", "h2", "h3"]:
                    if next_element.name == "ul":
                        characters.extend(
                            self.parse_character_list(next_element, section_text)
                        )
    ...
    def parse_character_list(
        self, ul_element, section_context: str
    ) -> List[Dict[str, Any]]:
    ...
    def run_full_scrape(self):
        ...
        for i, vehicle in enumerate(vehicles[:20]):  # Limit to first 20 for demo
            print(
                f"Enhancing vehicle {i+1}/{min(20, len(vehicles))}: {vehicle['name']}"
            )
        ...
        for i, character in enumerate(characters[:30]):  # Limit to first 30 for demo
            print(
                f"Enhancing character {i+1}/{min(30, len(characters))}: {character['name']}"
            )
    ...
    def get_database_stats(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT faction, COUNT(*) FROM gijoe_vehicles GROUP BY faction"
                )
                ...
                cursor.execute(
                    "SELECT faction, COUNT(*) FROM gijoe_characters GROUP BY faction"
                )
